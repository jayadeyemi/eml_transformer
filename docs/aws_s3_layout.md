...
`test`, the application key `bronze/source=weather_alerts/records.jsonl`
becomes `s3://<bucket>/test/bronze/source=weather_alerts/records.jsonl`.
...
Use these checks after a test or dev run to confirm data landed in the expected
folders:
...
- There is no automated integration test that asserts the full S3 folder
  contract against LocalStack or a temporary AWS bucket.
- `config-render-from-outputs` renders live AWS resources and, when supplied
  `--deployment`, merges source/path/model settings for local test commands.
