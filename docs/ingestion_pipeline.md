...
The base profile owns machine-level defaults and its selected source
list. The deployment loader merges the base profile and then the final
deployment file. `infra.engine: cdk` identifies AWS deployments. Runtime values
are rendered with:
...
- Dev schedules are disabled by default; test deployments can use accelerated
  schedules during the full AWS test and disable them after diagnostics.
