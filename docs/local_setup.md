...
and embedding settings so local test commands can run from the generated file.
...
Infrastructure parameters live in the YAML configs under
`configs/deployments/`, base profiles in the configs root, and source layers
under `configs/sources/`. Nothing is hardcoded in the CDK stack.
To update any infrastructure setting:
...
For local development and bounded tests against real AWS resources:
...
Run this against the deployment config you want to exercise before any real data
collection. The runner derives stack names, generated runtime config paths,
CloudFormation output paths, image tags, and ECR tags from `DEPLOYMENT_CONFIG`.
...
# 1 - inspect and validate the selected deployment
...
# 3 - synth
cdk synth -c deployment_config="$DEPLOYMENT_CONFIG" -c aws_profile=eml-dev

# 4 - deploy infrastructure
cdk deploy -c deployment_config="$DEPLOYMENT_CONFIG" -c aws_profile=eml-dev
cd ../..

# 5 - get runtime config from CDK outputs
eml_transformer config-render-from-outputs \
    --stack <stack_name from deployment-info> \
    --profile eml-dev \
    --deployment "$DEPLOYMENT_CONFIG" \
    --output <runtime_config_path from deployment-info>

# 6 - build and push the image used by Batch
ACCOUNT_ID=$AWS_ACCOUNT_ID
REGION=us-east-1
IMAGE_VERSION_TAG=<deployment_name from deployment-info>
REPO_URL="$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/<stack_name from deployment-info>-collection"

aws ecr get-login-password --region "$REGION" --profile eml-dev \
    | docker login --username AWS --password-stdin "$ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com"

docker build -t "eml-transformer:$IMAGE_VERSION_TAG" .
docker tag "eml-transformer:$IMAGE_VERSION_TAG" "$REPO_URL:$IMAGE_VERSION_TAG"
docker push "$REPO_URL:$IMAGE_VERSION_TAG"

# 7 - run the full AWS test suite
export NEWSAPI_SECRET_NAME=<operator-created-secrets-manager-secret-name>
export CONFIRM_STACK_DELETE=<stack_name from deployment-info>
bash scripts/aws/run_all.sh --reset-stack

# 8 - tear down retained artifacts again when needed
CONFIRM_STACK_DELETE=<stack_name from deployment-info> bash scripts/aws/cleanup/reset_test_stack.sh
```

The suite writes logs under `artifacts/aws_test_results/`. If the selected
deployment declares `runtime_secrets.NEWSAPI_KEY`, AWS Batch reads `NEWSAPI_KEY`
from the Secrets Manager secret named by `NEWSAPI_SECRET_NAME`. SNS sends
workflow completion and failure messages when the selected deployment enables
notifications. Embedding tests are currently best-effort unless the image
includes the optional HPC/modeling dependencies.
